window.addEventListener('DOMContentLoaded',()=>{
    ...
    function starttutorial(){
        ...
        alert('당신은 다음과 같은 일을 할 수 있습니다. [채집],[개발],[건설],[전투],[생산*개발중],[치료*개발중],[도박*개발중]')
    ...
    function start_cheeryo(i,ii){
        cheeryo_player_bool[ii]=false
        for (var is=0;is<50;is++){
            setTimeout(()=>{
                if (math(all[i],build_thing[ii])<build_thing[ii].width/2){
                }else{
                    return
                }
            },100);
        }
        cheeryo_player_bool[ii]=true
        ...
        for (var i=0;i<all.length;i++){
            for (var ii=0;ii<build_thing.length;ii++){
                if (wheels.indexOf(build_thing[ii].id)>16){
                    if (math(all[i],build_thing[ii])<build_thing[ii].width/2 && cheeryo_player_bool[ii]){
                        start_cheeryo(i,ii)
                    }
                }
            }
    ...
    function roof_thing(){
        ...
        for (var i=0;i<build_thing.length;i++){
            for (var ii=0;ii<enemy.length;ii++){
                if (math(build_thing[i],enemy[ii])<enemy[ii].sagury && ifhitss){
        ...
        for (var i=0;i<build_thing.length;i++){
            for (var ii=0;ii<enemy.length;ii++){
                if (math(build_thing[i],enemy[ii])<build_thing[i].sagury && ifhits){
    ...
    function createbuild(listener){
        ...
        if (wheels.indexOf(listener['id'])>16){
